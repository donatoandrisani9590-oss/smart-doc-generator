...
def renumber_clause_sections(clauses: list[dict]) -> list[dict]:
    ...
    The numbering is based on position among enabled §-clauses only.
    """
    # Pattern to detect if content already starts with a § heading (legacy data)
...
async def generate_document_by_type(
    ...
    try:
        ...
        # Check if user template should be used (clause-based injection)
            ...
            # Generate into user template (preserves header/footer/branding)
            docx_path = inject_content_into_template(
                template_path=template_file,
            ...
            # Standard generation (build document from scratch)
