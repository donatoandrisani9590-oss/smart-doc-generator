...
- Default: Mistral AI (servers in EU)
- Local Dev: Ollama (no data leaves machine)
- Sensitive data never sent to US providers by default
"""
import os
import json
import httpx
...
class LLMService:
    ...
    2. Mistral AI (if API key configured - best for production)
    3. OpenAI (fallback only, requires explicit opt-in)

    Usage:
        llm = LLMService()
        response = await llm.chat([
            LLMMessage(role="system", content="Du bist ein Rechtsexperte."),
            LLMMessage(role="user", content="Prüfe diese Klausel...")
        ])
    """
    ...
    async def _get_client(self) -> BaseLLMClient:
        ...
        # Auto-detect: Ollama first (local dev), then Mistral
        if await self.ollama.is_available():
            self._active_client = self.ollama
